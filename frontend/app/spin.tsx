...
export default function SpinWheelScreen() {
  ...
  const handleSpin = () => {
    ...
    // Add current rotation + extra spins + the specific angle for the winning segment
    const targetRotation = rotation.value + 360 * extraSpins + (360 - winningIndex * segmentAngle);

    rotation.value = withTiming(targetRotation, { duration: 4000, easing: Easing.out(Easing.cubic) }, (finished) => {
      if (finished) {
        // Must use runOnJS or similar for state update, but here we can just use setTimeout since it's 4s
      }
    });
    ...
    <SafeAreaView className="flex-1 bg-[#0F1115]">
      ...
      <View className="flex-1 items-center justify-center p-6 relative">
        ...
        <Animated.View entering={FadeInDown.delay(200)} className="relative mb-8 z-10 items-center justify-center">
          ...
          <Animated.View style={animatedStyle} className="w-80 h-80 bg-[#1A1D24] rounded-full border-4 border-white/10 items-center justify-center">
        ...
        <Animated.View entering={FadeInDown.delay(400)} className="mt-8 items-center relative z-10 w-full max-w-sm">
          <View className="bg-[#1A1D24] border border-white/10 rounded-2xl p-4 w-full">
            <View className="flex-row items-center justify-center space-x-2 mb-2 gap-2">
            ...
            <View className="flex-row justify-between w-full">
              {rewards.slice(0, 4).map((reward, index) => (
                <View key={index} className="bg-[#0F1115] rounded-xl p-2 items-center flex-1 mx-1">
